...
def log_event(
    ...
    if type_ in _INTEGRITY_REISSUE and attempt.status == "active":
        ...
        # отримує ПОВНІСТЮ новий варіант.
        #
        # Статус "voided" — тобто анульована спроба НЕ рахується проти
        # max_attempts. Спершу тут було "restarted" (рахувалося), щоб вихід не
        # став способом «перекинути» невдалу спробу. Практика це спростувала:
        # через перебої зі світлом і зв'язком вкладка ховається не з вини учня,
        # і студенти втрачали всі спроби, жодного разу не дійшовши до кінця.
        # Захист від списування не вартий того, щоб блокувати сумлінних.
